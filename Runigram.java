...
public class Runigram {
	...
	private static Color luminance(Color pixel) {
		...
		double calculation = 0.299 * r + 0.587 * g + 0.114 * b;
		int lum = (int) calculation;
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for(int i = 0; i < result.length; i++){
			for(int j = 0; j < result[0].length; j++){
				int changeH = (int)(i * newHeight);
				int changeW = (int)(j * newWidth);
				result[i][j] = image[changeH][changeW];
			}
		}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for(int i = 0; i <= n; i++){
			Color[][] result = blend(source, target, ((n - i) / (double) n));
			display(result);
			StdDraw.pause(500);
		}
	}
